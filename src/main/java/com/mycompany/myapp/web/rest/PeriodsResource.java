...
public class PeriodsResource {
    ...
    @GetMapping("/periods")
    @Timed
    public ResponseEntity<List<PeriodsDTO>> getAllPeriods(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("teacher-is-null".equals(filter)) {
            log.debug("REST request to get all Periodss where teacher is null");
            return new ResponseEntity<>(periodsService.findAllWhereTeacherIsNull(),
                    HttpStatus.OK);
        }
        log.debug("REST request to get a page of Periods");
        Page<PeriodsDTO> page = periodsService.findAll(pageable);
    ...
    @Timed
    public ResponseEntity<List<PeriodsDTO>> searchPeriods(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Periods for query {}", query);
        Page<PeriodsDTO> page = periodsService.search(query, pageable);
